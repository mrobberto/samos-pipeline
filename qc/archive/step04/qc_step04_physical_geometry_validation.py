...
PURPOSE
-------
Validate the physical slit geometry reconstructed by Step04 by comparing the
fitted center/edge models against the actual quartz trace image and the slitid
map.

This QC goes beyond simple summary statistics by checking whether the fitted
geometry follows the measured slit illumination row by row and whether the
trace masks remain continuous and physically plausible.

INPUTS
------
Expected products in config.ST04_PIXFLAT:
...
OUTPUTS
-------
Written under config.ST04_PIXFLAT:

- qc_step04_even/
- qc_step04_odd/

Each QC directory contains:
- per-slit PNG overlays and residual plots
- summary CSV
- summary text report

CHECKS PERFORMED
----------------
1) Overlay fitted center and edges on the quartz trace image

2) Compare raw vs fitted:
   - center position
   - left edge
   - right edge

3) Inspect slit width as a function of detector Y

4) Measure fragmentation:
   - number of disconnected slit segments per row
   - missing rows in the slitid map

5) Rank slits by residuals / fragmentation to identify the worst cases

INTERPRETATION
--------------
Good geometry should show:
- fitted center and edges following the quartz trace closely
- small row-by-row residuals
- smooth width variation
- low fragmentation

Warning signs:
- large center/edge residuals
- mask fragmentation
- abrupt width changes
- many missing rows in the slitid map

RUN
---
Spyder example:

    runfile("qc_step04_physical_geometry_validation.py")

NOTES
-----
- This script does not modify any pipeline products.
- It is intended as the main QC companion for Step04 geometry validation.
"""
...
def _make_slit_plot(
    ...
    fig.suptitle(f"Step04 QC — {slit_name}", fontsize=13)
...
def qc_one_set(trace_set: str):
    ...
    outdir = st04 / f"qc_step04_{trace_set.lower()}"
    ...
    with fits.open(geom_path) as hdul:
        for h in hdul[1:]:
            ...
            out_png = outdir / f"{base}_{slit_name}_qc.png"
            _make_slit_plot(
                out_png=out_png,
                trace_img=trace_img,
                sid=sid,
    ...
    # summary CSV
    csv_path = outdir / f"{base.lower()}_qc_summary.csv"
    if summary_rows:
    ...
    # text report
    txt_path = outdir / f"{base.lower()}_qc_report.txt"
    with open(txt_path, "w") as f:
        f.write(f"Step04 QC report — {trace_set.upper()}\n")
    ...
    print(f"[DONE] {trace_set.upper()} Step04 QC written to: {outdir}")
