#!/usr/bin/env python3
"""
qc_step11_summary_recovered_adopted.py
...
def _choose_display_spectrum(tab: fits.FITS_rec, lam_col: str) -> Tuple[np.ndarray, np.ndarray, str, str]:
    ...
    Choose the best spectrum to display:
      1) FLUX_FLAM (absolutely calibrated) in black
      2) FLUX_TELLCOR_O2 (telluric-corrected) in red
      3) FLUX in red
...
def find_latest(path: Path, patterns):
    hits = []
    for pat in patterns:
        hits.extend(Path(path).glob(pat))
    if not hits:
        return None
...
def parse_args():
    class Args:
        ...
        st10 = Path(getattr(config, "ST10_TELLURIC", getattr(config, "ST09_TELLURIC", st11)))
        st06 = Path(config.ST06_SCIENCE)
        st04 = Path(config.ST04_PIXFLAT)

        extract = str(st11 / "Extract1D_fluxcal.fits")
        photcat = str(st11 / "slit_trace_radec_skymapper_all.csv")

        # Prefer the known Run-8 imaging coadd the user specified.
        _wcs_candidates = [
            Path(repo_root) / "calibration" / "sisi" / "Coadd_i_median_078-082_ff_flipx_wcs_manual.fits",
        ]
        sisi_dir = getattr(config, "SISI_DIR", None)
        if sisi_dir:
            _wcs_candidates = [
                Path(sisi_dir) / "Coadd_i_median_078-082_ff_flipx_wcs_manual.fits",
                Path(sisi_dir) / "Coadd_i_median_078-082_ff_wcs_manual.fits",
            ] + _wcs_candidates
        _wcs_guess = next((p for p in _wcs_candidates if p.exists()), None)
        wcs = str(_wcs_guess) if _wcs_guess is not None else ""

        _det_guess = None
        for cand in [
            st06 / "FinalScience_dolidze_ADUperS.fits",
            st06 / "FinalScience_dolidze_ADUperS_pixflatcorr_clipped.fits",
        ]:
            if cand.exists():
                _det_guess = cand
                break

        if _det_guess is None:
            hits = []
            for pat in ["FinalScience*.fits"]:
                hits.extend(st06.glob(pat))
            hits = [
                p for p in hits
                if ("tracecoords" not in p.name.lower()) and ("slitcoords" not in p.name.lower())
            ]
            if hits:
                hits = sorted(set(hits), key=lambda p: p.stat().st_mtime)
                _det_guess = hits[-1]

        detector = str(_det_guess) if _det_guess is not None else ""

        _tr_even = find_latest(st06, ["*EVEN*tracecoords*.fits", "*even*tracecoords*.fits"])
        _tr_odd  = find_latest(st06, ["*ODD*tracecoords*.fits", "*odd*tracecoords*.fits"])
        tracecoords = (
            (str(_tr_even) if _tr_even else "")
            + "|"
            + (str(_tr_odd) if _tr_odd else "")
        )

        _sl_even = find_latest(st06, ["*EVEN*slitcoords*.fits", "*even*slitcoords*.fits"])
        _sl_odd  = find_latest(st06, ["*ODD*slitcoords*.fits", "*odd*slitcoords*.fits"])
        slitcoords = (
            (str(_sl_even) if _sl_even else "")
            + "|"
            + (str(_sl_odd) if _sl_odd else "")
        )

        _m_even = st04 / "Even_traces_mask_reg.fits"
        _m_odd  = st04 / "Odd_traces_mask_reg.fits"
        if not _m_even.exists():
            _m_even = st04 / "Even_traces_mask.fits"
        if not _m_odd.exists():
            _m_odd = st04 / "Odd_traces_mask.fits"
        mask = (
            (str(_m_even) if _m_even.exists() else "")
            + "|"
            + (str(_m_odd) if _m_odd.exists() else "")
        )

        _g_even = st04 / "Even_traces_geometry.fits"
        _g_odd  = st04 / "Odd_traces_geometry.fits"
        geometry = (
            (str(_g_even) if _g_even.exists() else "")
            + "|"
            + (str(_g_odd) if _g_odd.exists() else "")
        )

        mask_mode = "rectified"
        mask_crop = "match_trace"
        show_edges = True
        search_halfwidth = 120
        mask_pad = 6

        out = str(st11 / "qc_step11" / "qc_step11_summary_pages.pdf")
...
def main():
    ...
    if (not str(wcs_fits).strip()) or (not wcs_fits.exists()):
        wcs_fits = None
    ...
    with fits.open(extract_fits) as h:
        ...
        with PdfPages(out_pdf) as pdf:
            for slit in slits:
                ...
                # postage stamp
                stamp = stamp_wcs = None
                if wcs_fits is not None:
                    try:
                ...
                if stamp is not None:
                    ...
                    axB.axis("off")
                    axB.text(0.5, 0.5, "No imaging cutout", ha="center", va="center")
                ...
                axC.axvline(float(getattr(args, "lambda_halpha", 656.3)), color="red", lw=1.0, alpha=0.8)
                ...
                axC.set_title(f"{slit} final calibrated spectrum")
                if ylab == "FLUX_FLAM":
