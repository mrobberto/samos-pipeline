...
def _choose_display_spectrum(tab: fits.FITS_rec, lam_col: str) -> Tuple[np.ndarray, np.ndarray, str, str]:
    ...
    Choose the best spectrum to display:
      1) FLUX_FLAM (final absolute calibration) in black
...
def parse_args():
    class Args:
        ...
        _wcs_guess = Path(config.SISI_DIR) / "Coadd_i_median_078-082_ff_wcs_manual.fits"
        if not _wcs_guess.exists():
            _wcs_guess = find_latest(Path(config.SISI_DIR), ["*wcs*.fits", "*.fits"])
        wcs = str(_wcs_guess) if _wcs_guess is not None and Path(_wcs_guess).exists() else ""
        ...
        out = str(st11 / "qc_step11" / "QC_summary_Run8_recovered.pdf")
    ...
    with fits.open(extract_fits) as h:
        ...
        with PdfPages(out_pdf) as pdf:
            for slit in slits:
                ...
                plt.axvline(x=656.3, color='r', label='H-alpha',ls='--',alpha=0.5)
