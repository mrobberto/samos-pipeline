...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar 25 08:00:20 2026

QC07d — inspect bright-line alignment across slits.

Inputs:
  - arc_1d_even/odd.fits            (Step07c)
  - arc_shifts_initial_even/odd.csv (Step07d)
...
run:   
> PYTHONPATH=. python qc/step07/QC07d\ —\ inspect\ bright-line\ alignment\ across\ slits..py --set EVEN
> PYTHONPATH=. python qc/step07/QC07d\ —\ inspect\ bright-line\ alignment\ across\ slits..py --set ODD
...
def main(argv=None):
    ...
    ap.add_argument("--arc1d", type=str, default=None,
                    help="Input arc_1d FITS. Default: config.ST07_WAVECAL / arc_1d_<set>.fits")
    ap.add_argument("--csv", type=str, default=None,
                    help="Input shift CSV. Default: config.ST07_WAVECAL / arc_shifts_initial_<set>.csv")
    ap.add_argument("--plot-all", action="store_true",
                    help="Plot all slits instead of a small subset")
    args = ap.parse_args(argv)

    trace_set = args.set.upper()
    suffix = trace_set.lower()

    arc1d_fits = Path(args.arc1d).expanduser() if args.arc1d else (
        Path(config.ST07_WAVECAL) / f"arc_1d_{suffix}.fits"
    )
    csv_path = Path(args.csv).expanduser() if args.csv else (
        Path(config.ST07_WAVECAL) / f"arc_shifts_initial_{suffix}.csv"
    )
