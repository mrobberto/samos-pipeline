...
"""
QC07c — overlay Step04 trace geometry on the full-frame Step07b arc image.

Purpose
-------
Visualize the Step04 quartz-derived trace geometry directly on the
pixflat-corrected full-frame arc image used for wavelength calibration.

This QC is intended to diagnose whether the intended arc ridge is still
the one being captured by the Step04 slit geometry, or whether neighboring
brighter traces are intruding into the footprint.
...
- extract a geometry-centered cutout from the arc image
- overplot:
    centerline (cyan)
...
Outputs
-------
Written under config.ST07_WAVECAL:

- qc_step07_overlay_even/
- qc_step07_overlay_odd/
...
Run
---
    PYTHONPATH=. python qc/step07/qc07c_overlay_step04_geometry_on_arc.py --traceset EVEN
    PYTHONPATH=. python qc/step07/qc07c_overlay_step04_geometry_on_arc.py --traceset ODD
"""
...
def pick_arc_file(traceset: str, explicit: str | None = None) -> Path:
    if explicit:
    ...
    # preferred: canonical Step07b file derived from MASTER_ARC_DIFF
    if hasattr(config, "MASTER_ARC_DIFF"):
        stem = Path(config.MASTER_ARC_DIFF).stem
        cand = st07 / f"{stem}_pixflatcorr_clipped.fits"
        if cand.exists():
            return cand

    # fallback: newest matching ArcDiff pixflatcorr product
    hits = sorted(st07.glob("ArcDiff*_pixflatcorr_clipped.fits"), key=lambda q: q.stat().st_mtime, reverse=True)
    if hits:
        return hits[0]

    raise FileNotFoundError(f"No Step07b arc image found under {st07}")
...
def main():
    ap = argparse.ArgumentParser(description="Overlay Step04 trace geometry on Step07b full-frame arc image")
    ap.add_argument("--traceset", required=True, choices=["EVEN", "ODD", "even", "odd"])
    ap.add_argument("--arc", default=None, help="Optional explicit Step07b full-frame arc FITS")
    ...
    arc_path = pick_arc_file(traceset, args.arc)

    outroot = Path(args.outdir).expanduser() if args.outdir else (Path(config.ST07_WAVECAL) / f"qc_step07_overlay_{traceset.lower()}")
    outroot.mkdir(parents=True, exist_ok=True)

    tag = f"{arc_path.stem}_with_{geom_path.stem}"
    subdir = outroot / tag
    subdir.mkdir(parents=True, exist_ok=True)

    print("[INFO] Arc image:", arc_path)
    print("[INFO] Geometry :", geom_path)

    arc = fits.getdata(arc_path).astype(float)
