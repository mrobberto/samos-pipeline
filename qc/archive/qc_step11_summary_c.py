...
Patched behavior:
- auto-discovers EVEN/ODD TRACECOORDS from config when --tracecoords is omitted
- auto-discovers imaging file from config or calibration/sisi when --image is omitted
- prefers config.EXTRACT1D_FLUXCAL and config.STEP11_PHOTCAT if available
- keeps headless plotting
"""
...
def _auto_tracecoords_arg() -> str:
    even = getattr(config, "SCI_EVEN_TRACECOORDS", None)
    odd = getattr(config, "SCI_ODD_TRACECOORDS", None)
    if even and odd:
        return f"{even}|{odd}"
    if even:
        return str(even)
...
def _auto_image_path() -> str:
    # Prefer explicit config variables if present
    for name in [
        "IMAGING_FITS", "IMAGE_FITS", "REFERENCE_IMAGE", "COADD_FITS",
        "SISI_COADD_FITS", "CALIB_IMAGE_FITS"
    ]:
        p = getattr(config, name, None)
        if p and Path(p).exists():
            return str(Path(p))

    # Then try calibration/sisi canonical files under repo
    repo_root = Path(getattr(config, "REPO_ROOT", Path(__file__).resolve().parents[2]))
    cands = [
...
def parse_args():
    default_extract = str(getattr(config, "EXTRACT1D_FLUXCAL", Path(config.ST11_FLUXCAL) / "Extract1D_fluxcal.fits"))
    default_photcat = str(
        getattr(config, "STEP11_PHOTCAT",
                getattr(config, "PHOTCAT_CSV",
                        Path(config.ST11_FLUXCAL) / "slit_trace_radec_skymapper_all.csv"))
    )
    default_tracecoords = _auto_tracecoords_arg()
    default_image = _auto_image_path()
    default_outpdf = str(Path(config.ST11_FLUXCAL) / "qc_step11" / "qc_step11_summary_pages.pdf")

    ap = argparse.ArgumentParser(description="Step11 QC summary pages")
    ap.add_argument("--extract", default=default_extract)
    ap.add_argument("--photcat", default=default_photcat)
    ap.add_argument("--tracecoords", default=default_tracecoords)
    ap.add_argument("--image", default=default_image)
    ap.add_argument("--slits", default="")
    ap.add_argument("--halfsize", type=int, default=30)
    ap.add_argument("--outpdf", default=default_outpdf)
...
def main():
    ...
    with fits.open(extract_fits) as hspec, PdfPages(outpdf) as pdf:
        ...
        if args.slits:
            wanted = [norm_slit(x) for x in args.slits.split(",")]
            slits = [s for s in wanted if s in available]
        else:
            slits = available
        ...
        for slit in slits:
            ...
            if timg is not None:
                ...
                ax0.set_xlabel("x (rectified spatial pix)")
                ax0.set_ylabel("y (binned rows)")
            else:
                ax0.axis("off")
        ...
        ax.text(
            ...
            f"TRACECOORDS: {args.tracecoords or 'None'}\n"
            f"Image file : {str(image_path) if image_path is not None else 'None'}\n"
