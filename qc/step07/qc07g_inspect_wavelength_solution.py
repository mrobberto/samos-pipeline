...
Default behavior is headless: figures are saved only when --save-prefix is
provided, and never shown on screen. Use --show-plots only if interactive
display is explicitly desired.
...
def finish_figure(args, outfile: str | None = None):
    if outfile:
...
def main(argv=None):
    ...
    if peak_matches is not None:
        ...
        plt.ylabel("Count")
        plt.tight_layout()
        outfile = f"{args.save_prefix}_master_residuals.png" if args.save_prefix else None
        finish_figure(args, outfile)

    arc1d = load_arc1d_flux_map(arc1d_even, arc1d_odd)

    ywin0 = int(mhdr["YWIN0"]) if "YWIN0" in mhdr else 1225
    firstlen = int(mhdr["FIRSTLEN"]) if "FIRSTLEN" in mhdr else 2875
    ...
    plt.tight_layout()
    outfile = f"{args.save_prefix}_slit_spectra_wavelength.png" if args.save_prefix else None
    finish_figure(args, outfile)
