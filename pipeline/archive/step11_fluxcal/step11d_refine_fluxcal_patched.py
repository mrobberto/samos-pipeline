...
This step provides an optional refinement of the Step11c flux calibration by
enforcing consistency with broadband photometry on a slit-by-slit basis.

Philosophy
----------
Step11c defines the baseline absolute flux calibration and is considered the
primary science product. Step11d does not replace this calibration, but refines
it when sufficient photometric constraints are available.

Rather than deriving a single global correction, Step11d operates independently
on each slit. This avoids imposing a common response shape on all spectra and
preserves slit-specific behavior.

Method
------
For each slit:

1. The Step11c spectrum (FLUX_FLAM) is read.
2. Broadband constraints are constructed using SkyMapper r, i, z photometry.
3. To ensure consistency with the spectral coverage:
   - the r band is truncated on the blue side ("r_long")
   - the z band is truncated on the red side ("z_short")
   - the i band is used unchanged
4. Synthetic magnitudes corresponding to these modified bandpasses are derived
   from the catalog r/i/z photometry.
5. A low-order polynomial response function is solved that matches the
   band-integrated fluxes to the photometric constraints.
6. The correction is applied only if the solution passes quality criteria
   (conditioning, positivity, and consistency checks).

If the fit is not accepted, the original Step11c flux is retained.

Outputs
-------
For each slit, the output includes:

- FLUX_FLAM : original Step11c calibrated spectrum
- FLUX_FLAM_REFINED : refined spectrum (or identical to FLUX_FLAM if rejected)
- RESP_STEP11D : multiplicative response relative to FLUX_FLAM
- diagnostic quantities describing the fit quality and acceptance
...
# ----------------------------------------------------------------------
def make_r_short_filter(wr: np.ndarray, tr: np.ndarray, lam_min: float) -> Tuple[np.ndarray, np.ndarray]:
    m = wr >= lam_min
    wrs = wr[m].copy()
    trs = tr[m].copy()
...
def synth_band_integral(lam_nm: np.ndarray,
                        flam: np.ndarray,
    ...
    if np.count_nonzero(m) < 3:
        return np.nan
    return trapz(flam[m] * ti[m] * lam_nm[m], lam_nm[m])
...
def solve_one_slit_exact(
    ...
    if bandpass_mode == "edge_matched":
        # Gold-standard 1slittester logic:
        # use fixed cuts, not observed-coverage-derived cuts.
        ...
        mag_pts = np.array([mags_full.get("r", np.nan),
                            mags_full.get("i", np.nan),
                            mags_full.get("z", np.nan)], float)

        m_r = synth_abmag_from_fnu_model(wrs, trs, lam_pts, mag_pts)
        m_z = synth_abmag_from_fnu_model(wzs, tzs, lam_pts, mag_pts)
...
def process_file(infile: Path,
    ...
    with fits.open(infile) as hdul:
        ...
        for idx in table_hdus:
            ...
            wave_nm_raw = np.asarray(wave_full[good], float)
            flux_raw = np.asarray(flux_full[good], float)
            
            order = np.argsort(wave_nm_raw)
            wave_nm = wave_nm_raw[order]
            flux = flux_raw[order]

            mags_full = phot_lookup[slit_id]

            try:
#                coeff, response, lambda0_nm, scale_nm, residuals, bandpasses_local_solver, mags_work = solve_one_slit_exact(
                coeff, response, lambda0_nm, scale_nm, residuals, _, mags_work, order = solve_one_slit_exact(    
                    wave_nm=wave_nm,
                    flux=flux,
                    mags_full=mags_full,
            ...
            x, _, _ = choose_scaled_coordinate(wave_nm, lambda0_nm, scale_nm)
            rows = []
            for band in ("r", "i", "z"):
                if not _is_finite_mag(mags_work[band]):
                    continue
                wb, tb = bandpasses[band] if bandpass_mode == "full" else (
                    make_r_short_filter(*bandpasses["r"], lam_min=R_SHORT_CUT_NM) if band == "r"
                    else bandpasses["i"] if band == "i"
                    else make_z_short_filter(*bandpasses["z"], lam_max=Z_SHORT_CUT_NM)
                )
                tran = interp_bandpass(wave_nm, wb, tb)
                rows.append(build_design_row(flux, wave_nm, tran, x))
            ...
            flux_refined = flux * response
            
            # --- REBUILD bandpasses locally (robust fix) ---
            if bandpass_mode == "edge_matched":
                wr, tr = bandpasses["r"]
                wi, ti = bandpasses["i"]
                wz, tz = bandpasses["z"]
            
                wr_work, tr_work = make_r_short_filter(wr, tr, lam_min)
                wz_work, tz_work = make_z_short_filter(wz, tz, lam_max)
                ...
                bandpasses_local = bandpasses
            
            # --- synthetic fluxes (Step11c) ---
            syn_r = synth_band_flam_mean(wave_nm, flux, bandpasses_local["r"])
            syn_i = synth_band_flam_mean(wave_nm, flux, bandpasses_local["i"])
            syn_z = synth_band_flam_mean(wave_nm, flux, bandpasses_local["z"])
            
            # --- interpolated transmissions ---
            tran_r = interp_bandpass(wave_nm, *bandpasses_local["r"])
            tran_i = interp_bandpass(wave_nm, *bandpasses_local["i"])
            tran_z = interp_bandpass(wave_nm, *bandpasses_local["z"])
            
            # --- targets ---
            tgt_r = observed_band_target_ab(mags_work["r"], wave_nm, tran_r) if _is_finite_mag(mags_work["r"]) else np.nan
            tgt_i = observed_band_target_ab(mags_work["i"], wave_nm, tran_i) if _is_finite_mag(mags_work["i"]) else np.nan
            tgt_z = observed_band_target_ab(mags_work["z"], wave_nm, tran_z) if _is_finite_mag(mags_work["z"]) else np.nan
            ...
            # --- refined flux ---
            syn_r_ref = synth_band_flam_mean(wave_nm, flux_refined, bandpasses_local["r"])
            syn_i_ref = synth_band_flam_mean(wave_nm, flux_refined, bandpasses_local["i"])
            syn_z_ref = synth_band_flam_mean(wave_nm, flux_refined, bandpasses_local["z"])
            ...
            if mode in ("perstar", "both"):
                ...
                response_unsorted = np.empty_like(response)
                response_unsorted[order] = response
            
                full_response = np.full(len(tab), np.nan, dtype=float)
                full_response[good] = response_unsorted
                new_tab["RESP_STEP11D"] = full_response
            
                # convert output fluxes
                full_flux_cgs = np.asarray(tab[local_flux_col], dtype=float).copy() * 1e-6
                full_flux_ref_cgs = full_flux_cgs.copy()
                if accepted:
                    full_flux_ref_cgs[good] *= response_unsorted
            
                new_tab["FLUX_FLAM"] = full_flux_cgs
                new_tab["FLUX_FLAM_REFINED"] = full_flux_ref_cgs
    
            if local_var_col is not None:
                full_var_cgs2 = np.asarray(tab[local_var_col], dtype=float).copy() * (1e-6)**2
                full_var_ref_cgs2 = full_var_cgs2.copy()
                if accepted:
                    full_var_ref_cgs2[good] *= response_unsorted**2

                new_tab["VAR_FLAM2"] = full_var_cgs2
                new_tab["VAR_FLAM2_REFINED"] = full_var_ref_cgs2

            new_tab["FLUX_FLAM"].unit = "erg / (s cm2 Angstrom)"
            new_tab["FLUX_FLAM_REFINED"].unit = "erg / (s cm2 Angstrom)"
            if local_var_col is not None:
                new_tab["VAR_FLAM2"].unit = "erg2 / (s2 cm4 Angstrom2)"
                new_tab["VAR_FLAM2_REFINED"].unit = "erg2 / (s2 cm4 Angstrom2)"
                    
                # Write output fluxes in cgs: erg s^-1 cm^-2 A^-1
                full_flux_cgs = np.asarray(tab[local_flux_col], dtype=float).copy() * 1e-6
                full_flux_ref_cgs = full_flux_cgs.copy()
                if accepted:
                    full_flux_ref_cgs[good] *= response

                new_tab["FLUX_FLAM"] = full_flux_cgs
                new_tab["FLUX_FLAM_REFINED"] = full_flux_ref_cgs

                # If variance exists, convert to cgs^2
                if local_var_col is not None:
                    full_var_cgs2 = np.asarray(tab[local_var_col], dtype=float).copy() * (1e-6)**2
                    full_var_ref_cgs2 = full_var_cgs2.copy()
                    if accepted:
                        full_var_ref_cgs2[good] *= response**2

                    new_tab["VAR_FLAM2"] = full_var_cgs2
                    new_tab["VAR_FLAM2_REFINED"] = full_var_ref_cgs2

                # Set table-column units
                new_tab["FLUX_FLAM"].unit = "erg / (s cm2 Angstrom)"
                new_tab["FLUX_FLAM_REFINED"].unit = "erg / (s cm2 Angstrom)"
                if local_var_col is not None:
                    new_tab["VAR_FLAM2"].unit = "erg2 / (s2 cm4 Angstrom2)"
                ...
                hdr["BUNIT"] = ("erg s-1 cm-2 A-1", "Flux density unit for FLUX_FLAM columns")
        ...
        if mode in ("master", "both"):
            ...
            for idx in table_hdus:
                ...
                full_flux_cgs = np.asarray(tab[local_flux_col], dtype=float).copy() * 1e-6
                full_flux_ref_cgs = full_flux_cgs.copy()
                gmask = np.isfinite(full_flux_ref_cgs) & np.isfinite(response_full)
                full_flux_ref_cgs[gmask] *= response_full[gmask]
                ...
                if local_var_col is not None:
                    full_var_cgs2 = np.asarray(tab[local_var_col], dtype=float).copy() * (1e-6)**2
                    full_var_ref_cgs2 = full_var_cgs2.copy()
                    gmaskv = np.isfinite(full_var_ref_cgs2) & np.isfinite(response_full)
                    full_var_ref_cgs2[gmaskv] *= response_full[gmaskv] ** 2
        ...
        stem = infile.stem
        if mode in ("master", "both"):
            hdul_out_master.writeto(outdir / f"{stem}_refined_master.fits", overwrite=True)
        if mode in ("perstar", "both"):
            hdul_out_perstar.writeto(outdir / f"{stem}_refined_perstar.fits", overwrite=True)
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--bandpass-mode", choices=["full", "edge_matched"], default="edge_matched",
                   help="Use original r/i/z filters or tester-style fixed-cut r_short/i/z_short")
...
def main() -> None:
    ...
    if args.mode in ("perstar", "both"):
        print("[OK] Wrote", outdir / f"{stem}_refined_perstar.fits")
