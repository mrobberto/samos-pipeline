...
Each SLIT### extension is a binary table with columns:
    YYPIX, FLUX, VAR, SKY, OBJ_PRESKY, X0, NOBJ, NSKY, SKYSIG,
...
def extract_obj_presky(img, x0, sky_sig, TEXP):
    ny, nx = img.shape
    x = np.arange(nx, dtype=float)
    fin = np.isfinite(img)

    obj_presky = np.full(ny, np.nan, float)
    var_presky = np.full(ny, np.nan, float)
    nobj = np.zeros(ny, int)
    ...
    for y in range(ny):
        if (not np.isfinite(x0[y])) or fin[y].sum() < 5:
            continue
        ...
        invV = np.zeros(nx, float)
        invV[obj_mask] = 1.0 / V[obj_mask]

        denom = np.sum((P[obj_mask] ** 2) * invV[obj_mask])
        if denom <= 0 or (not np.isfinite(denom)):
            continue

        numer = np.sum(P[obj_mask] * row[obj_mask] * invV[obj_mask])

        obj_presky[y] = numer / denom
        var_presky[y] = 1.0 / denom

    return obj_presky, var_presky, nobj
...
def main():
    ...
    with fits.open(IN_FITS, memmap=False) as hdul:
        ...
        for h in hdul[1:]:
            ...
            obj_presky, var_presky, nobj = extract_obj_presky(img, x0, sky_sig, exptime)
